...
RUN wget https://get.docker.com/builds/Linux/x86_64/docker-1.5.0 && mv docker-1.5.0 /usr/local/bin/docker && chmod +x /usr/local/bin/docker
...
ADD sudoers /etc/
ADD https://storage.googleapis.com/golang/go1.5.2.linux-amd64.tar.gz /usr/local/
RUN cd /usr/local && tar -xvf go1.5.2.linux-amd64.tar.gz && rm go1.5.2.linux-amd64.tar.gz && mv go go1.5.2 && ln -s go1.5.2 go
USER jenkins
